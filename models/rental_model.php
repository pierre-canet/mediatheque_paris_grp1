...
// Correction : Vérification du succès de l'opération au lieu de se fier à db_last_insert_id
function create_rental($user_id, $item_id) {
    ...
    try {
        ...
        $result = db_execute($query, [$user_id, $item_id]);
        update_item_availability($item_id, 0);
        db_commit();
        return $result; // Retourne le résultat de l'exécution de la requête au lieu de db_last_insert_id
    } catch (Exception $e) {
        db_rollback();
        return false;
    }
...
// Correction : Utilisation des nouveaux noms de tables (movies, video_games)
function get_user_rentals($user_id) {
    $query = "SELECT r.*, i.title, i.type, i.image_url 
              FROM rentals r 
              JOIN (
                  SELECT CONCAT('book_', id) AS id, title, 'book' AS type, image_url FROM books
                  UNION ALL
                  SELECT CONCAT('film_', id) AS id, title, 'film' AS type, image_url FROM movies
                  UNION ALL
                  SELECT CONCAT('game_', id) AS id, title, 'game' AS type, image_url FROM video_games
              ) i ON r.item_id = i.id 
...
// Correction : Utilisation de l'ID combiné pour la mise à jour
