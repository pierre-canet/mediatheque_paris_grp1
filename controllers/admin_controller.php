...
function admin_media_save($id = null) {
    ...
    if ($type === 'livre') {
        ...
        $extra_fields['date_of_publication'] = intval($_POST['date_of_publication'] ?? 0);
    } elseif ($type === 'film') {
    ...
    if (!in_array($type, ['livre', 'film', 'jeu'])) $errors[] = "Type invalide";
    ...
    // Persistance
    if ($id) {
        update_media($id, $title, $type, $genre, $stock, $extra_fields);
        set_flash_message('success', 'Média mis à jour avec succès.');
    } else {
        create_media($title, $type, $genre, $stock, $extra_fields);
...
function admin_media_delete($id) {
    require_admin();
    if (delete_media($id)) {
...
function admin_loan_create($user_id, $media_id) {
    require_admin();
    if (create_loan($user_id, $media_id)) {
