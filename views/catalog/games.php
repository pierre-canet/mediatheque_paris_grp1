<?php
// Affichage de la page des jeux vidéo
...
<head>
    ...
    <title><?php echo htmlspecialchars($title ?? 'Jeux Vidéo'); ?></title>
...
<body>
    <section class="container">
        ...
        <section class="search-filters">
            <!-- Formulaire de recherche avec méthode GET pour envoyer les paramètres à catalog/games -->
            <form method="GET" action="<?php echo url('catalog/games'); ?>">
                <!-- Barre de recherche pour la saisie du titre et les boutons -->
                <div class="search-bar">
                    <!-- Champ de saisie pour la recherche du titre du jeu -->
                    <input type="text" name="search_term" placeholder="Rechercher un jeu vidéo..." class="search-input" value="<?php echo htmlspecialchars($search_term ?? ''); ?>">
                    ...
                    <?php if (!empty($search_term) || ($search_genre ?? 'all') != 'all' || ($search_availability ?? 'all') != 'all'): ?>
                        ...
                        <a href="<?php echo url('catalog/games'); ?>" class="btn btn-secondary" style="margin-left: 10px;">Effacer</a>
                    <?php endif; ?>
                </div>
                ...
                <div class="filters">
                    ...
                    <div class="filter-group">
                        ...
                        <!-- Menu déroulant pour sélectionner le genre du jeu -->
                        <select id="genre" name="genre" class="filter-select">
                            ...
                            <!-- Options spécifiques de genre pour les jeux vidéo -->
        ...
        <div class="grid-container">
            <?php if (empty($items)): ?>
                <p class="no-results">Aucun jeu vidéo trouvé.</p>
            <?php else: ?>
                <?php foreach ($items as $item): ?>
                    <div class="item">
                        ...
                        <p>Éditeur : <?php echo htmlspecialchars($item['writer_producer_editor']); ?></p>
                        <p>Plate-forme : <?php echo htmlspecialchars($item['ISBN13_classification_platform']); ?></p>
                        <p>Âge minimum : <?php echo htmlspecialchars($item['page_number_duration_min_age']); ?></p>
        ...
        <div class="pagination">
            ...
            <?php if ($current_page > 1): ?>
                <a href="<?php echo url('catalog/games?page=' . ($current_page - 1)); ?>" class="btn btn-prev-page">Page précédente</a>
            <?php endif; ?>

            <!-- Liens numériques des pages -->
            <div class="page-numbers">
                <!-- Toujours afficher la page 1 -->
                <a href="<?php echo url('catalog/games?page=1'); ?>" class="page-number <?php echo $current_page == 1 ? 'active' : ''; ?>">1</a>
                ...
                for ($i = $start_page; $i <= $end_page; $i++): ?>
                    <a href="<?php echo url('catalog/games?page=' . $i); ?>" class="page-number <?php echo $i == $current_page ? 'active' : ''; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>
            ...
            <?php if ($current_page < $total_pages): ?>
                <a href="<?php echo url('catalog/games?page=' . ($current_page + 1)); ?>" class="btn btn-next-page">Page suivante</a>
            <?php endif; ?>
        </div>
